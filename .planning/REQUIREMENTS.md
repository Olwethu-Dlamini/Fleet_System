...
- [ ] **FOUND-01**: Add `tenant_id` column to all tables for multi-tenant isolation
- [ ] **FOUND-02**: Fix race condition in job assignment — wrap availability check + insert in `SELECT ... FOR UPDATE` transaction
- [ ] **FOUND-03**: Fix job number generation race condition — use atomic sequence table
...
- [ ] **FOUND-07**: Set `TZ=UTC` in Docker and add `tenant_timezone` field
- [ ] **FOUND-08**: Fix `GROUP_CONCAT` truncation — set `group_concat_max_len=65536` per connection
- [ ] **FOUND-09**: Add database indexes on `scheduled_date`, `current_status`, `tenant_id`
